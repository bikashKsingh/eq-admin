...
export function PaymentSettingList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/paymentSettings?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/paymentSettings/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/paymentSettings`, recordId);
    } else {
      apiResponse = await remove(`/paymentSettings/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Payment Settings</h4>
            </div>
            <div>
              <Link
                title="Alt + '+'"
                to={"/paymentSettings/add"}
                type="button"
