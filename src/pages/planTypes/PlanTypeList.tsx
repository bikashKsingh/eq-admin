...
export function PlanTypeList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/planTypes?page=${pagination.page}&limit=${pagination.limit}`;
  ...
  const columns: Column<Record>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ value }: any) => {
          return value == true ? (
            <span className="badge bg-info rounded">Yes</span>
          ) : (
            <span className="badge bg-danger rounded">No</span>
          );
        },
      },
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/planTypes`, recordId);
    } else {
      apiResponse = await remove(`/planTypes/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Plan Types</h4>
            </div>
            <div>
              <Link
                title="Alt + '+'"
