...
export function KycDocumentList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/kycDocuments?page=${pagination.page}&limit=${pagination.limit}`;
  ...
  const columns: Column<Record>[] = React.useMemo(
    () => [
...

      {
        ...
        accessor: "formats",
        Cell: ({ value }) => {
          return value.map(({ format }) => format).join(", ");
        },
      },
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/kycDocuments/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/kycDocuments`, recordId);
    } else {
      apiResponse = await remove(`/kycDocuments/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">KYC Document List</h4>
            </div>
            <div>
              <Link
                title="Alt + '+'"
