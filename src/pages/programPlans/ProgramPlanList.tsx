...
export function ProgramPlanList() {
        ...
        Cell: ({ value }: any) => {
          return (
            <p className="d-flex gap-2">
              <del className="text-danger">₹{value.mrp}</del>
              <span className="text-success">₹{value.salePrice}</span>
            </p>
          );
        },
      },
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/programPlans/edit/${value}`,
                }}
              >
              ...
              <Link
                className="p-2 bg-light"
                to={{
                  pathname: `/programPlans/details/${value}`,
                }}
              >
                <span
  ...
  const data = React.useMemo(() => {
    return records.map((data) => {
      return {
        ...
        plan: data.plan?.name,
        planDuration: `${data.planDuration} Days`,
        priceInr: {
          salePrice: data.salePriceInr,
          mrp: data.mrpInr,
        },
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/programPlans`, recordId);
    } else {
      apiResponse = await remove(`/programPlans/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Program Plan List</h4>
            </div>
            <div>
              <Link
                to={"/plans/add"}
