...
export function TrainerDesignationList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/trainerDesignations?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/trainerDesignations/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/trainerDesignations`, recordId);
    } else {
      apiResponse = await remove(`/trainerDesignations/${recordId}`);
    }
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      if (event.altKey && event.key === "=") {
        navigate("/trainerDesignations/add");
      }
    };
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Trainer Designation</h4>
            </div>
            <div>
              <Link
                title="Alt + '+'"
                to={"/trainerDesignations/add"}
                type="button"
