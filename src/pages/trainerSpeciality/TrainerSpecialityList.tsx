...
export function TrainerSpecialityList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/trainerSpecialities?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/trainerSpecialities/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/trainerSpecialities`, recordId);
    } else {
      apiResponse = await remove(`/trainerSpecialities/${recordId}`);
    }
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      if (event.altKey && event.key === "=") {
        navigate("/trainerSpecialities/add");
      }
    };
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Trainer Speciality</h4>
            </div>
            <div>
              <Link
                title="Alt + '+'"
                to={"/trainerSpecialities/add"}
                type="button"
