...
export function BookingTimeSlotList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/bookingTimeSlots?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/bookingTimeSlots/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/bookingTimeSlots`, recordId);
    } else {
      apiResponse = await remove(`/bookingTimeSlots/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Booking Time Slot List</h4>
            </div>
            <div>
              <Link
                title="Alt + '+'"
                to={"/bookingTimeSlots/add"}
                type="button"
                className="btn btn-primary text-light"
              >
                Add Booking Time Slot
              </Link>
            </div>
          </div>
        </div>
