...
export function TimeSloatList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/timeSlots?page=${pagination.page}&limit=${pagination.limit}&displayOrder=ASC`;
  ...
  const columns: Column<Record>[] = React.useMemo(
    () => [
      ...
      {
        Header: "GROUP AVAILABLE",
        accessor: "isGroupAvailable",
        Cell: ({ value }) => {
          return value ? (
  ...
  const data = React.useMemo(() => {
    return records.map((data) => {
      return {
        ...
        isGroupAvailable: data.isGroupAvailable,
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/timeSlots`, recordId);
    } else {
      apiResponse = await remove(`/timeSlots/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Time Slots</h4>
            </div>
            <div>
              <Link
                to={"/timeSlots/add"}
