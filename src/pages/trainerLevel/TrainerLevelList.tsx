...
export function TrainerLevelList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/trainerLevels?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/trainerLevels/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/trainerLevels`, recordId);
    } else {
      apiResponse = await remove(`/trainerLevels/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Trainer Level</h4>
            </div>
            <div>
              <Link
                title="Alt + '+'"
                to={"/trainerLevels/add"}
                type="button"
