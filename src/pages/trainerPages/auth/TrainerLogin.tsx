...
export function TrainerLogin(): ReactNode {
  ...
  const { values, errors, touched, handleChange, handleBlur, handleSubmit } =
    useFormik({
      ...
      onSubmit: async (value, helpers) => {
        setLoading(true);
        const apiResponse = await post("/trainers/login", value);

        if (apiResponse?.status == 200) {
          const token = apiResponse?.body?.token;
          dispatch({ type: "SET_TOKEN", payload: { token, user: "TRAINER" } });
            ...
            <div className="row w-100 mx-0">
              <div className="col-lg-4 mx-auto">
                <div className="auth-form-light text-left py-5 px-4 px-sm-5">
                  ...
                  <div className="mb-3">
                    <h4 className=" text-dark login-title">TRAINER LOGIN</h4>
                  </div>
