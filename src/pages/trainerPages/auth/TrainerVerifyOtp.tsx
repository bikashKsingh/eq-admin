...
export function TrainerVerifyOtp(): ReactNode {
  ...
  const { values, errors, touched, handleChange, handleBlur, handleSubmit } =
    useFormik({
      ...
      onSubmit: async (value, helpers) => {
        ...
        const apiResponse = await post("/trainers/verifyOtp", {
          ...value,
          email,
        });
        ...
        if (apiResponse?.status == 200) {
          ...
          navigate("/trainer/create-new-password");
        } else {
  ...
  async function handleResendOtp(evt: React.MouseEvent) {
    ...
    // get email
    const email = localStorage.getItem("email");
    const apiResponse = await post("/trainers/findAccount", {
      email,
    });
    if (apiResponse?.status == 200) {
