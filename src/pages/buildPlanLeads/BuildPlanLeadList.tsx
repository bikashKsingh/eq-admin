...
export function BuildPlanLeadList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/buildPlanLeads?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/buildPlanLeads/edit/${value}`,
                }}
              >
              ...
              <Link
                className="p-2 bg-light"
                to={{
                  pathname: `/buildPlanLeads/details/${value}`,
                }}
              >
                <span
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/buildPlanLeads`, recordId);
    } else {
      apiResponse = await remove(`/buildPlanLeads/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Build Plan Leads</h4>
            </div>
            <div>
              {/* <Link
                to={"/buildPlanLeads/add"}
                type="button"
