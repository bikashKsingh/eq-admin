...
export function NutritionConsultationDetails() {
  const navigate = useNavigate();
  const { bookingId, userId } = useParams();
  const [loading, setLoading] = useState<boolean>(true);
  const [bookingDetails, setBookingDetails] = useState<any>(null);
  const [onboardingRegDetails, setNutritionConsultationDetails] =
    useState<any>(null);
    ...
    function () {
      async function getData(userId: string) {
        ...
        const apiResponse = await get(
          `/nutritionConsultations?user=${userId}`,
          true
        );
        if (apiResponse?.status == 200) {
          if (apiResponse?.body?.length) {
            setNutritionConsultationDetails(apiResponse?.body[0]);
          }
        }
          ...
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              ...
              <h4 className="font-weight-bold mb-0">
                Nutrition Consultation Details
              </h4>
            </div>
            {/* <div>
              <Link
