...
export function BudgetList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/budgets?page=${pagination.page}&limit=${pagination.limit}&displayOrder=ASC`;
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/budgets`, recordId);
    } else {
      apiResponse = await remove(`/budgets/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Budgets</h4>
            </div>
            <div>
              <Link
                title="Alt + '+'"
