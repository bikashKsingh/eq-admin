...
export function ProgramRequirementList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/programRequirements?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/programRequirements/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/programRequirements`, recordId);
    } else {
      apiResponse = await remove(`/programRequirements/${recordId}`);
    }
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      if (event.altKey && event.key === "=") {
        navigate("/programRequirements/add");
      }
    };
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Program Requirements</h4>
            </div>
            <div>
              <Link
                title="Alt + '+'"
                to={"/programRequirements/add"}
                type="button"
