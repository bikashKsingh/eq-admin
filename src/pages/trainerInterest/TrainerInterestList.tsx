...
export function TrainerInterestList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/trainerInterests?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/trainerInterests/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/trainerInterests`, recordId);
    } else {
      apiResponse = await remove(`/trainerInterests/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Trainer Interests</h4>
            </div>
            <div>
              <Link
                title="Alt + '+'"
                to={"/trainerInterests/add"}
                type="button"
