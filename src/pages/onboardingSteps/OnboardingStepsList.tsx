...
export function OnboardingStepsList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/onboardingSteps?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/onboardingSteps/edit/${value}`,
                }}
              >
              ...
              <Link
                className="p-2 bg-light"
                to={{
                  pathname: `/onboardingSteps/details/${value}`,
                }}
              >
                <span
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/onboardingSteps`, recordId);
    } else {
      apiResponse = await remove(`/onboardingSteps/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Onboarding Steps</h4>
            </div>
            <div>
              <Link
                to={"/onboardingSteps/add"}
                type="button"
